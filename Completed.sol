...
contract GhospTokenVesting is Context, Ownable {
    ...
    mapping(uint256 => VestingSchedule) public vestingScheduleList;
    uint256 numberOfSchedulesList;
    ...
    uint256 public initialTotalAmount = 75000000 ether;
    ...
    function initialData() private {
        ...
        //add schedule list
        for(uint256 i = 0; i < numberOfSchedulesList; i++){
            if(i == 0){
                vestingScheduleList[i] = VestingSchedule(
                    5, //%
                    vestingTimeList[i] //vestingtime
                );
            }else {
                vestingScheduleList[i] = VestingSchedule(
                    19, //%
        ...
        for (uint256 j = 1; j < numberOfSchedulesList; j++) {
    ...
    function unlockToken() external onlyAdmin {
        ...
        require(releasedVestingId < 27, "Lock period End and All locked tokens were unlocked");
        ...
        for (uint256 i = 0; i < members.length; i++) {
            ...
            if (!vestingAddress.isSent) {
                require(_token.transfer(members[i], vestingAddress.amount));
            ...
            if (members[i] == _msgSender()) {
                for (uint256 j = 0; j < vestingTimeList.length; j++) {
                    if (currentTime >= vestingTimeList[j] && !vestingTimeScheduleList[vestingTimeList[j]][_msgSender()].isSent) {
                        require(_token.transfer(_msgSender(), vestingTimeScheduleList[vestingTimeList[j]][_msgSender()].amount));
