...
import { cspdTokenAddress, vestingContractAddress, busdTokenAddress, usdtTokenAddress } from '../../contract_ABI/vestingData';
import { 
    useGetTierOfAccount,
...
const BuyModal = ({ isOpen, setIsOpen, onlyOneToast}) => {
    ...
    // const busdBalance = Number(useTokenBalance(busdTokenAddress, account) / 10 ** 18).toFixed(2);
    ...
    useEffect( () => {
        setBuyCspdAmount(Number(buyUsdAmount * 1000 / 8).toFixed(2));
        setTier(maxAmountOfTier ? Number((maxAmountOfTier*0.008)).toFixed(2) : 0);
    ...
    function handleBuy() {
        ...
        console.log('clicked the buy button', Math.round(buyCspdAmount), payCurrency);
    }
